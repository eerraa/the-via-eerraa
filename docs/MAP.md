...
공통 규약(헤더 두 줄, 장르 다섯, 색인 세 열, 최소 검사 path·header·index·citation)의
정본은 [eerraa-agent-docs](https://github.com/eerraa/eerraa-agent-docs) 태그 **v1**의
[`AGENT_DOCS_CONVENTION.md`](https://github.com/eerraa/eerraa-agent-docs/blob/v1/AGENT_DOCS_CONVENTION.md)다.
규약을 여기 복제하지 않는다. 이 저장소가 보태는 것만 적는다.
...
- 경로·상수·링크·스크립트와, 규약이 요구하는 path·header·index·citation은
